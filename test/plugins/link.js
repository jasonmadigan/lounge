...
describe("Link plugin", function() {
	...
	it("should find og:image with full url", function(done) {
		...
		this.app.get("/thumb", function(req, res) {
			res.send("<title>Google</title><meta property='og:image' content='http://localhost:9002/favicon.png'>");
		});

		this.irc.once("msg:preview", function(data) {
			assert.equal(data.preview.head, "Google");
			assert.equal(data.preview.thumb, "http://localhost:9002/favicon.png");
			done();
		});
	});
	...
	it("should not use thumbnail with invalid url", function(done) {
		...
		this.app.get("/invalid-thumb", function(req, res) {
			res.send("<title>test</title><meta property='og:image' content='/favicon.png'>");
		});
	...
	it("should send untitled page if there is a thumbnail", function(done) {
		...
		this.app.get("/thumb-no-title", function(req, res) {
			res.send("<meta property='og:image' content='http://localhost:9002/favicon.png'>");
		});
	...
	it("should send image preview", function(done) {
		const message = this.irc.createMessage({
			text: "http://localhost:9002/favicon.png"
		});
		...
		this.irc.once("msg:preview", function(data) {
			...
			assert.equal(data.preview.link, "http://localhost:9002/favicon.png");
			done();
		});
	});
});
